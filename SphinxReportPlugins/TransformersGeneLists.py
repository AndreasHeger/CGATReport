...
class TransformerHypergeometric( Transformer ):
    ...
    def transform(self, data, path):
        ...
        if len(keys) < 3 or "background" not in data.keys():
            raise ValueError ("Expected at least 3 lists, with one called background, instead got %i lists called %s" % (len(keys), ", ".join(keys)))


        keys = [x for x in keys if x != "background"]

        missing = {y:[str(x) for x in data[y] if x not in data["background"]] for y in keys}


        if any([len(missing[x]) >0 for x in missing]):
            missing_items = "\n\t".join([ "%s:\t%s" % (x,",".join(missing[x])) for x in missing])
            raise ValueError("Found items in lists not in background. Missing items:\n\t %s" % missing_items)
...
class TransformerOddsRatio( Transformer ):
    ...
    def transform(self, data, path):
        ...
        if len(keys)==2:
            ...
            d = len( set(data["background"]) - set(data[keys[0]]) - set(data[keys[1]]))
...
class TransformerVenn( Transformer ):
    '''Takes dictionarys of lists and transforms the data into the correct format for
    use with the venn renderer. If the dictionary contains a background entry and
    all other entries are subsets of the background, then it is removed, unless the
    keep-background options is specified.
    ...
    def transform(self, data, path):
        ...
        if len(data)==2:
            ...
            raise ValueError ("Can currently only cope with 2 or 3 way intersections")
